...
const ExamCard = React.memo(({ exam }) => {
  ...
  return (
    <li className="exam-card-item">
      ...
      <button onClick={toggle} className="toggle-btn" aria-expanded={open}>
      ...
      {activeCategory && (
        <div className="exam-category-card">
          <h3 className="exam-card-title">
            ...
            {!loading && <span className="exam-count-tag">{exams.length} Found</span>}
          </h3>

          {loading && <Loading />}

          {!loading && exams.length === 0 && (
            <div className="no-exams-wrap">
              <span className="no-exams-tag">Nil</span>
              <p className="no-exams-found">No exams found for {activeCategory}</p>
            </div>
          )}
